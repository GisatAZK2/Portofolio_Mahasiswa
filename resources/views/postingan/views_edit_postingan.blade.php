        ...
        <div class="p-8">
            ...
            <form method="POST" action="{{ route('postingan.update', ['id' => $postingan->id_postingan]) }}"
                ...
                <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="mt-4" id="thumbnail_container" style="display: {{ old('game_enabled', $hasGame) ? 'block' : 'none' }};">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ autoTranslate('Thumbnail Game (opsional)') }}
                        </label>
                        @if($gameThumbnail)
                            <div class="mb-3" id="current_thumbnail_container">
                                <img src="{{ asset('storage/' . $gameThumbnail) }}" class="max-w-xs h-auto rounded-lg border border-gray-200 dark:border-gray-700"
                                    alt="Current game thumbnail" id="current_thumbnail">
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    {{ autoTranslate('Thumbnail saat ini') }}
                                </p>
                                <button type="button" id="remove_thumbnail_btn" 
                                    class="text-xs text-red-600 hover:text-red-800 mt-1">
                                    {{ autoTranslate('Hapus thumbnail') }}
                                </button>
                        ...
                        <input type="file" name="game_thumbnail" id="game_thumbnail" accept="image/*"
                            class="mt-2 block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-50 dark:file:bg-indigo-900 file:text-indigo-700 dark:file:text-indigo-300 hover:file:bg-indigo-100 dark:hover:file:bg-indigo-800"
                            {{ (old('game_enabled', $hasGame)) ? '' : 'disabled' }}>
                        <p class="text-xs text-gray-500 mt-1">{{ autoTranslate('Maks 5MB • jpg, jpeg, png, gif, webp') }}</p>
                    </div>
                </div>
