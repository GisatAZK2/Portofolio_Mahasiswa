...
@section('content')
    <style>
        ...
        .dark .comment-item:hover { background: rgba(255,255,255,0.03); }
        ...
        .dark .replies-container { border-left-color: #374151; }
        ...
        .edit-form textarea, .reply-form-container textarea { transition: all 0.2s ease; font-size: 0.875rem; }

        .comment-text { word-break: break-word; white-space: pre-wrap; line-height: 1.5; }

        .comment-loading {
            display: inline-block; width: 18px; height: 18px;
            border: 2px solid #e5e7eb; border-top: 2px solid #6366f1;
        ...
        .btn-loading::after {
            ...
            top: 50%; left: 50%; margin-left: -7px; margin-top: -7px;
        ...
        /* ── Search result info bar (shown below section title) ── */
                ...
                <div class="feed-column">
                    ...
                    <div class="mb-6 flex justify-between items-center flex-wrap gap-3">
                        <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100"
                            data-translate="perihal_terbaru" data-translate-page="dashboard">
                            {{ autoTranslate('Postingan Terbaru') }}
                        </h2>
                    ...
                    <div class="feed-section mb-10">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <h3 class="text-lg font-semibold text-indigo-700 dark:text-indigo-300"
                                data-translate="ur_post" data-translate-page="dashboard">
                        ...
                        <div id="dashboard-search-result-info"
                            ...
                            <span>
                                <span id="dashboard-search-count" class="font-semibold text-indigo-600 dark:text-indigo-400">0</span>
                                {{ autoTranslate('postingan ditemukan') }}
                            </span>
                        </div>
                                    ...
                                    @foreach($postinganTerbaru as $post)
                                        ...
                                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden border border-gray-200 dark:border-gray-700 post-card relative"
                                            ...
                                            <div class="p-4 cursor-pointer" onclick="window.location.href='{{ route('postingan.show', ['locale' => app()->getLocale(), 'id' => $post->id_postingan]) }}'">
                                                ...
                                                @endif
                                                @if($deskripsi)
                                                    <p class="text-[20px] text-gray-600 dark:text-gray-300 line-clamp-4">{{ autoTranslate(Str::limit(strip_tags($deskripsi), 180)) }}</p>
                                                @endif
                                                    ...
                                                    <div class="mt-2 p-3 border border-gray-100 dark:border-gray-800 rounded-lg flex items-center justify-between">
                                                        <div class="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                @if($game->score > 0)
                                                                    <div class="text-xs text-green-600 dark:text-green-400 mt-1">🏆 {{ autoTranslate('Skor terbaik') }}: {{ $game->score }}</div>
                                                                @endif
                                                            </div>
                                                        </div>
                                                        @auth
