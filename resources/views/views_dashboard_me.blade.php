            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($randomPosts as $post)
                        <!-- Card wrapper -->
                        <div class="min-h-[320px]flex flex-col h-full rounded-xl overflow-hidden border border-gray-100 shadow-md hover:shadow-xl transition-all duration-30 {{ $post->type !== 'sertifikat' ? 'group-hover:border-indigo-300 group-hover:ring-1 group-hover:ring-indigo-200' : '' }}">
                            @php
                                $cardHref = '#'; // default
                                $isExternal = false;
