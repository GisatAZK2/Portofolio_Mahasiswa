                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-gray-200 dark:border-gray-700 mb-8">
                    <div class="flex items-center gap-4">
                        @if(auth()->user()->photo_profile && file_exists(public_path('storage/' . auth()->user()->photo_profile)))
                            <img src="{{ asset('storage/' . auth()->user()->photo_profile) }}" class="w-12 h-12 rounded-full object-cover" alt="">
                        @else
                    ...
                    <div class="flex gap-4 mt-4 justify-around">
                        <button onclick="openCreatePostModal()" class="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-blue-600 transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                            <span class="text-sm font-medium">Foto</span>
                        </button>
                        <button onclick="openCreatePostModal()" class="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-red-600 transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @foreach($postinganTerbaru as $post)
                                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden border border-gray-200 dark:border-gray-700 post-card">
                                        ...
                                        <div class="p-4 cursor-pointer" onclick="window.location.href='{{ route('postingan.show', ['locale' => app()->getLocale(), 'id' => $post->id_postingan]) }}'">
                                            ...
                                            @endif
                                            @if($imageUrl)
                                                <div class="mb-4 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-700 w-auto h-auto shadow-sm mt-3">
                                        ...
                                        <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 flex items-center justify-between">
                                            ...
                                            <span onclick="window.location.href='{{ route('postingan.show', ['locale' => app()->getLocale(), 'id' => $post->id_postingan]) }}'" class="text-xs text-gray-500 dark:text-gray-400 cursor-pointer hover:text-indigo-600">Lihat detail →</span>
                                        </div>
                                            ...
                                            @auth
                                                <form action="{{ route('komentar.store') }}" method="POST" class="mb-4">
                                                    ...
                                                    <div class="flex gap-3">
                                                        ...
                                                        @endif
                                                        <div class="flex-1">
                                                            <textarea name="komentar" rows="2" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none resize-y" placeholder="Tulis komentar..."></textarea>
    ...
    <div class="modal-content">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white dark:bg-gray-800 z-10">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Bagikan pemikiran, cerita, atau pengalaman Anda</p>
            </div>
            <button type="button" onclick="closeCreatePostModal()" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 transition">
        ...
        <div class="p-6 overflow-y-auto" style="max-height: calc(90vh - 140px);">
            <form method="POST" action="{{ route('postingan.store') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4" class="w-full px-4 py-3 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 placeholder-gray-500 shadow-sm bg-white">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between mb-5">
                        ...
                        <button type="button" id="add-item" class="inline-flex items-center px-4 py-2 text-sm font-medium bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition">
                ...
                <div class="flex justify-end pt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                    <button type="button" onclick="closeCreatePostModal()" class="px-6 py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        Batal
                    </button>
                    <button type="submit" class="px-8 py-2.5 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition">
                        Buat Postingan
                    </button>
                </div>
            </form>
...
<script>
    window.addEventListener('load', function() {
        setTimeout(function() {
            ...
            document.getElementById('learning-skeleton-sidebar').classList.add('hidden');
            document.getElementById('learning-content-wrapper-sidebar').classList.remove('hidden');
    ...
    function addItem() {
        ...
        newItem.innerHTML = `
            <div class="flex justify-between items-start mb-4">
                ...
                <button type="button" class="remove-item text-red-500 hover:text-red-700 text-sm font-medium">Hapus</button>
            </div>
            <div class="content-area">
                <textarea name="items[${itemIndex}][content]" rows="3" class="text-input hidden w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:border-indigo-500 outline-none transition"
    ...
    function createSparkline(canvasId, borderColor) {
        ...
        const generateRandomData = () => Array.from({ length: 7 }, () => Math.floor(Math.random() * 40) + 10);
