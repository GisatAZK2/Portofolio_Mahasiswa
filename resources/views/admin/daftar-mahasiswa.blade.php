    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6">
        ...
        <div id="clientPaginationControls" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p class="text-sm text-gray-600 dark:text-gray-400">
                <span data-translate="show" data-translate-page="admin">Menampilkan</span> <span
                    id="paginationVisibleCount">0</span> <span data-translate="from" data-translate-page="admin">dari</span>
                <span id="paginationTotalCount">{{ $users->count() }}</span> <span data-translate="user"
                    data-translate-page="admin">pengguna</span>.
            </p>
...
@push('scripts')
    <script>
        ...
        function getExportData() {
            const users = [];
            // Select all paginated items (including hidden for pagination, because all data exists in DOM)
        ...
        function getMobileEmailText(row) {
            ...
            if (emailDiv && emailDiv.textContent !== '...') {
                return emailDiv.textContent;
            }
            const emailData = row.querySelector('.email-cell-mobile')?.getAttribute('data-email');
            return emailData || '-';
        }
        ...
        function exportToWord() {
            ...
            <html>
            <head><meta charset="UTF-8"><title>Data Pengguna</title>
            ...
            <body>
                ...
                <table>
                    <thead><tr><th>NO</th><th>NAMA LENGKAP</th><th>USERNAME</th><th>EMAIL</th><th>ROLE</th><th>STATUS PENGAJUAN</th><th>STATUS AKTIF</th><th>JURUSAN/PRODI</th><th>ANGKATAN</th></tr></thead>
                    <tbody>`;
        ...
        function toggleIndividualEmailVisibility(button, isMobile = false) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.email-toggle-btn').forEach(btn => {
                btn.addEventListener('click', function(e) {
                    e.stopPropagation();
                    toggleIndividualEmailVisibility(this, false);
                });
            });
            ...
            document.querySelectorAll('.email-toggle-btn-mobile').forEach(btn => {
                btn.addEventListener('click', function(e) {
                    e.stopPropagation();
                    toggleIndividualEmailVisibility(this, true);
                });
            });
            ...
            const selectAllCheckbox = document.getElementById('selectAllCheckbox');
            if (selectAllCheckbox) selectAllCheckbox.addEventListener('change', function() { toggleAll(this, false); });

            const tableSelectAllCheckbox = document.getElementById('tableSelectAllCheckbox');
            if (tableSelectAllCheckbox) tableSelectAllCheckbox.addEventListener('change', function() { toggleAll(this, true); });

            @if (session('success'))
                Swal.fire({ icon: 'success', title: 'Berhasil!', text: '{{ session('success') }}', showConfirmButton: false, timer: 3000, timerProgressBar: true });
            @endif
            @if (session('error'))
                Swal.fire({ icon: 'error', title: 'Gagal!', text: '{{ session('error') }}', confirmButtonColor: '#dc2626' });
            @endif
            @if (session('info'))
                Swal.fire({ icon: 'info', title: 'Informasi', text: '{{ session('info') }}', confirmButtonColor: '#3b82f6' });
            @endif
        });
    </script>
    <script>
        document.addEventListener("DOMContentLoaded", () => { showPageInfo("popup.manage_users"); });
    </script>
@endpush