...
function toggleSeeMore(gridClass, button, totalCount) {
    ...
    const itemClass = gridClass.replace('-grid', '-item');
    const items = Array.from(grid.querySelectorAll(`.${itemClass}`));
    const hiddenItems = items.filter(item => item.classList.contains('hidden'));
    const nextChunk = hiddenItems.slice(0, 20);

    nextChunk.forEach(item => item.classList.remove('hidden'));

    if (hiddenItems.length <= 20) {
