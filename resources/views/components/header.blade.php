<header class="bg-white/70 backdrop-blur-md border-b border-gray-200/50 sticky top-0 z-50 transition-all">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-4 lg:hidden">
                <button id="toggle-sidebar" class="text-gray-700 focus:outline-none">
                    ...
                    <svg id="sidebar-close" class="w-7 h-7 hidden transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
                    ...
                    <div class="flex gap-2 shrink-0">
                        <button type="submit" class="bg-indigo-600 text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 transition text-sm font-medium flex items-center gap-1.5 shadow-sm">
        ...
        <div class="px-4 py-5 space-y-5 sm:px-6">
            <form method="GET" action="{{ route('search') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <select name="jurusan" class="block w-full border border-gray-300/80 rounded-lg py-3 px-3.5 text-sm bg-white/80 backdrop-blur-sm focus:ring-indigo-400 focus:border-indigo-400">
                        <option value="">Semua Jurusan</option>
                        @foreach($jurusanList ?? [] as $jurusan)
                            <option value="{{ $jurusan->id_jurusan }}" {{ request('jurusan') == $jurusan->id_jurusan ? 'selected' : '' }}>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:gap-4">
                    <button type="submit" class="flex-1 bg-indigo-600 text-white py-3.5 rounded-lg text-sm font-medium hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 transition flex items-center justify-center gap-2 shadow-sm">
                        <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
                        Cari
                    </button>
                    <a href="{{ route('search') }}" class="flex-1 bg-white/60 text-gray-700 py-3.5 rounded-lg text-sm font-medium hover:bg-gray-100/80 focus:ring-2 focus:ring-gray-300 transition flex items-center justify-center border border-gray-300/50 backdrop-blur-sm">
                        Reset Filter
                    </a>
                </div>
            </form>
