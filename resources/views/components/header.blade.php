        ...
        <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-3 lg:hidden">
                @auth
                    ...
                    @endif
                    <!-- User Guide Button (Mobile) -->
                    @if(Auth::user()->role == 'mahasiswa')
                        <button id="userGuideButtonMobile"
                            class="text-gray-700 dark:text-gray-300 focus:outline-none p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-all duration-200"
                            title="Panduan Pengguna"
                            onclick="window.showUserGuide && window.showUserGuide()">
                ...
                @auth
                    ...
                    <div x-data="notificationBell({ userId: {{ Auth::id() }}, userRole: '{{ Auth::user()->role }}' })" x-init="init()" class="relative">
                        <button type="button" @click="toggleDropdown" class="relative p-2 bg-white/80 dark:bg-gray-800/80 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full shadow-md transition-all">
                            ...
                            <span x-cloak x-show="unreadCount > 0" x-text="unreadCount > 99 ? '99+' : unreadCount" class="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white animate-pulse"></span>
                        </button>
                        <div x-show="isOpen" x-cloak @click.away="isOpen = false" x-transition class="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden z-50">
