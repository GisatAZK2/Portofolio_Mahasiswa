    ...
    <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
        ...
        <div class="space-y-1">
            <button onclick="toggleProjectMenu()" class="w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('project.*') ? 'bg-blue-600 text-white shadow-md' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-700' }}">
                ...
                <svg id="projectArrow" class="w-4 h-4 transition-transform duration-300 {{ request()->routeIs('project.*') && Auth::user()->id == request()->route('id') ? 'rotate-180' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>

            <div id="projectMenu" class="pl-4 space-y-1 {{ request()->routeIs('project.*') && Auth::user()->id == request()->route('id') ? '' : 'hidden' }} mt-1">
                   @auth    
            <a href="{{ route('project.index') }}" class="flex items-center space-x-3 px-5 py-2.5 rounded-lg text-sm transition-all duration-200 {{ request()->routeIs('project.index') ? 'bg-blue-100 text-blue-800 font-medium' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-700' }}">
                ...
                <a href="{{ route('project.create', Auth::user()->id) }}" class="flex items-center space-x-3 px-5 py-2.5 rounded-lg text-sm transition-all duration-200 {{ request()->routeIs('project.create') ? 'bg-blue-100 text-blue-800 font-medium' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-700' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                    </svg>
