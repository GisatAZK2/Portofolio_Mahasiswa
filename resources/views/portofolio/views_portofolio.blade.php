                        ...
                        <div class="p-5 flex-1 flex flex-col">
                            ...
                            @if($link_project || $link_github || $link_video)
                                <div class="flex flex-wrap gap-3 text-sm mb-4">
                                    @if($link_project)
                                        <a href="{{ $link_project }}" target="_blank" class="text-indigo-600 hover:underline">Project</a>
                                    @endif
                                    @if($link_github)
                            ...
                            <div class="flex items-center justify-between mt-5 pt-4 border-t border-gray-100">
                                ...
                                <form class="delete-form"
                                      action="{{ route('portofolio.destroy', $porto->id_portfolio) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                            class="delete-btn text-sm px-4 py-1.5 bg-red-100 text-red-700 rounded-md hover:bg-red-200 transition">
                                        Delete
                                    </button>
                                </form>
                            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', async function(e) {
            ...
            if (confirmed) {
                showLoading('Menghapus portofolio...');
