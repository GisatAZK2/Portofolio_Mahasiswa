function initDosenProjectEditPage() {
    const dataEl = document.getElementById('dosen-project-edit-data');
    if (!dataEl) return;
    ...
    const updateFormInputs = () => {
        ...
        if (leaderInput) {
            const effectiveLeaderId = selectedUsers.leader?.id && selectedUsers.leader.id !== selectedUsers.owner?.id ? selectedUsers.leader.id : '';
            leaderInput.value = effectiveLeaderId;
        }
    ...
    window.removeSelectedUser = function (userId, role) {
        ...
        // Ini menyamakan perilaku UI dengan backend (handleTasks di DosenController), yang
        // otomatis membuang task milik user yang sudah tidak termasuk owner/leader/members.
            ...
            return `
                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    ...
                    <div class="flex items-center gap-2">
                        <select class="user-role-select px-3 py-1 border border-gray-300 dark:border-gray-500 rounded-lg text-sm" onchange="window.updateDosenProjectEditUserRole(this, ${user.id}, this.value)">
    ...
    window.updateDosenProjectEditUserRole = function (selectElement, userId, role) {
    ...
    window.updateUserRole = function (selectElement, userId, role) {
        window.updateDosenProjectEditUserRole(selectElement, userId, role);
    };
    ...
    const updateTaskSectionVisibility = () => {
        ...
        taskSection.classList.toggle('hidden', !(selectedUsers.owner || selectedUsers.leader || selectedUsers.members.length > 0));
    };
    ...
    // ----- Default filter based on dosen's keahlian -----
    if (keahlianSelect && dosenAttrs.id_keahlian) {
        keahlianSelect.value = dosenAttrs.id_keahlian;
    }
...
document.addEventListener('DOMContentLoaded', initDosenProjectEditPage);
document.addEventListener('turbo:load', initDosenProjectEditPage);